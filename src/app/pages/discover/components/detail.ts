    ...
    @if (discoverService.isSelectedBookLoading()) {
      ...
      @if (discoverService.selectedBook(); as book) {
        <div class="mx-auto max-w-5xl">
          <a routerLink="/discover" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-900 mb-6">
          ...
          <div class="flex flex-col gap-8 rounded-lg bg-white p-8 shadow-md md:flex-row">
            <div class="w-full md:w-1/3 flex-shrink-0">
              <img class="w-full rounded-lg object-cover shadow-lg" [src]="book.coverImageUrl || '[https://placehold.co/400x600/e2e8f0/64748b?text=Cover](https://placehold.co/400x600/e2e8f0/64748b?text=Cover)'" alt="Cover for {{ book.title }}">
            </div>
            <div class="flex flex-col">
              <div>
                @if (book.categories.length > 0) {
                  <p class="font-semibold text-indigo-600">{{ book.categories[0] }}</p>
                }
