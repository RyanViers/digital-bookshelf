  ...
  template: `
    @if (booksService.selectedBook(); as book) {
      <div class="mx-auto max-w-5xl">
        <button (click)="close()" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-300 hover:text-white mb-6">
        ...
        <div class="flex flex-col gap-8 rounded-lg bg-slate-800 border border-slate-700 p-8 shadow-md md:flex-row">
          <div class="w-full md:w-1/3 flex-shrink-0">
            ...
            <h1 class="mt-4 text-2xl font-bold tracking-tight text-white">{{ book.title }}</h1>
            <p class="mt-1 text-lg text-slate-300">by {{ book.author }}</p>
            <!-- Description Section -->
            <div class="mt-4 border-t border-slate-600 pt-4">
              <h2 class="text-sm font-medium text-slate-400">Description</h2>
              <p class="mt-1 text-sm text-slate-300">{{ book.description || 'No description available.' }}</p>
            </div>
          </div>
          <div class="w-full md:w-2/3">
            <form [formGroup]="editForm" (ngSubmit)="submitForm()" class="space-y-6">
              <div>
                <label for="status" class="block text-sm font-medium leading-6 text-white">Reading Status</label>
                <select formControlName="status" id="status" class="mt-2 block w-full rounded-md border-0 bg-slate-700 text-white p-2 shadow-sm ring-1 ring-inset ring-slate-600 focus:ring-2 focus:ring-indigo-500">
              ...
              <div>
                <label class="block text-sm font-medium leading-6 text-white">Your Rating</label>
                <div class="mt-2 flex items-center">
                  @for (star of [1, 2, 3, 4, 5]; track star) {
                    <button (click)="setRating(star)" type="button" class="text-slate-500 transition-colors hover:text-yellow-400">
              ...
              <div>
                <label for="notes" class="block text-sm font-medium leading-6 text-white">Review / Notes</label>
                <textarea formControlName="notes" id="notes" rows="6" class="mt-2 block w-full rounded-md border-0 bg-slate-700 text-white p-2 shadow-sm ring-1 ring-inset ring-slate-600 focus:ring-2 focus:ring-indigo-500 placeholder-slate-400"></textarea>
              </div>
              <div class="flex items-center justify-end gap-x-4 border-t border-slate-600 pt-6">
                <button (click)="promptDelete()" type="button" class="text-sm font-semibold leading-6 text-red-400 hover:text-red-300">Remove from Shelf</button>
